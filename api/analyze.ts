...
// Cache global para evitar duplicidade (Ação Corretiva NC 81-05)
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // --- 1. TRATAMENTO DE INTERAÇÃO (BOTAO TELEGRAM) ---
  ...
  // --- 2. CONFIGURAÇÕES DE TEMPO E STATUS (ITEM 7 & 6) ---
  ...
  try {
    // --- 4. PROCESSAMENTO PARALELO (Ação Corretiva NC 81-11) ---
    await Promise.all(ATIVOS.map(async (ativo) => {
      ...
      // Filtros de Segurança: Inclinação EMA e Cor da Vela (NC 81-02)
      let sinalStr = "";
      ...
      if (sinalStr) {
        ...
        if (!lastSinais[opId]) {
          ...
          await fetch(`https://api.telegram.org/bot${token}/sendMessage`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ chat_id, text: msg, parse_mode: 'HTML', reply_markup: teclado }) });
        }
      }
    }));

    // --- 5. INTERFACE HTML - REGRA DE OURO (ITEM 4 & 8) ---
    ...
    return res.status(200).send(`
      ...
      <style>
        ...
        <table class="revision-table">
          ...
          <tbody>
            <tr><td>83</td><td>15/02/26</td><td>12:30</td><td>Ações Corretivas NC 81 + Botões Semi-Auto</td></tr>
