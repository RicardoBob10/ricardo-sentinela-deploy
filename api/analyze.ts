...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Tenta executar a lógica, mas não deixa o erro matar o HTML
  try {
    const ativosTech = [
    ...
    for (const ativo of ativosTech) {
      ...
      const candles = json.map((v: any) => ({ t: v[0], c: parseFloat(v[4]) }));
      ...
      // Lógica de cruzamento Bullex (Ciano 9 / Roxa 21)
      if ((m9 > m21 && m9_ant <= m21_ant && rsi > 50) || (m9 < m21 && m9_ant >= m21_ant && rsi < 50)) {
        const opId = `${ativo.label}_${candles[i].t}`;
        if (!lastSinais[opId]) {
          lastSinais[opId] = true;
          const call = m9 > m21;
          const msg = `<b>SINAL EMITIDO!</b> ${call ? "🟢" : "🔴"}\n<b>ATIVO:</b> ${ativo.label}\n<b>SINAL:</b> ${call ? "↑ COMPRAR" : "↓ VENDER"}\n<b>PREÇO:</b> $ ${candles[i].c.toFixed(ativo.prec)}`;
    ...
    console.error("Erro na lógica, mas exibindo dashboard...");
  }

  // EXIBIÇÃO OBRIGATÓRIA DO SEU HTML (NÃO SAI DAQUI)
