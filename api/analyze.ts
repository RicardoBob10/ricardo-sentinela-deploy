...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Obter hora e minuto (Ex: 2341)
  const formatHora = new Intl.DateTimeFormat('pt-BR', { timeZone, hour: '2-digit', minute: '2-digit', hour12: false });
  const [h, m] = formatHora.format(agora).split(':').map(Number);
  const horaFormatada = h * 100 + m;

  // Obter dia da semana exato em SP (0=Dom, 1=Seg... 6=Sab)
  const formatDia = new Intl.DateTimeFormat('en-US', { timeZone, weekday: 'numeric' });
  const realDiaSemana = (parseInt(formatDia.format(agora)) % 7); 
