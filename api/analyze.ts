...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const getStatus = (label: string): boolean => {
    ...
    if (label === "EURUSD") {
      ...
      // SEGUNDA A QUINTA: Pausa das 18:01 às 21:59
  ...
  try {
    for (const ativo of ATIVOS) {
      ...
      try {
        ...
        // LÓGICA FRACTAL i-2 (RT_ROBO_V.01)
        ...
        if (sinalStr) {
          const opId = `v${versao}_${ativo.label}_${candles[i].t}_${sinalStr}`;
          if (!lastSinais[opId]) {
