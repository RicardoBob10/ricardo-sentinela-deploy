...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Obter hora e minuto numérico (Ex: 23:44 -> 2344)
  const formatTime = new Intl.DateTimeFormat('pt-BR', { timeZone, hour: '2-digit', minute: '2-digit', hour12: false });
  const timeParts = formatTime.formatToParts(agora);
  ...
  // Obter dia da semana exato em SP (0=Dom, 1=Seg... 6=Sab)
  // Usamos 'en-US' com numeric para evitar problemas de tradução de string
  const formatDay = new Intl.DateTimeFormat('en-US', { timeZone, weekday: 'numeric' });
  const realDiaSemana = (parseInt(formatDay.format(agora)) % 7); 

  const dataHoraStr = agora.toLocaleString('pt-BR', { timeZone });
  ...
  try {
    for (const ativo of ATIVOS) {
      ...
      if (sinalStr && !lastSinais[opId]) {
        const hVela = new Date(candles[i].t * 1000).toLocaleTimeString('pt-BR', { timeZone, hour: '2-digit', minute: '2-digit' });
