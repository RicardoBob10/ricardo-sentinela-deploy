...
// Memória temporária (Volátil em Serverless)
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // --- REGRAS DE ABERTURA CONFORME ITEM 5 ---
  ...
  try {
    for (const ativo of ATIVOS) {
      ...
      try {
        ...
        // i = Vela Atual (em formação), i-1 = Vela Fechada
        const i = candles.length - 1; 
        const rsi_val = calcularRSI(candles, i);
        const rsi_ant = calcularRSI(candles, i - 1);

        // LÓGICA FRACTAL i-2 (IDÊNTICA AO SCRIPT RT_ROBO_V.01)
        ...
        let sinalStr = "";
        // Validação da cor da vela atual (candles[i]) conforme script aprovado
        ...
        if (sinalStr) {
          ...
          // Chave de unicidade rígida (Ativo + Timestamp + Tipo) para evitar duplicidade
